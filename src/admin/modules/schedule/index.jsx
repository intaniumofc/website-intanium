...
export default function AdminSchedule() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    const data = await scheduleService.getEvents('all', 'All', { includeDrafts: true });
    setItems(data);
    setIsLoading(false);
  };
  ...
  const handlePublish = async (id) => {
    const res = await scheduleService.setEventStatus(id, 'published');
    if (res.success) {
      setItems(items.map(i => i.id === id ? { ...i, status: 'published' } : i));
      notify.success('Dipublikasikan', 'Jadwal sekarang tampil di website utama.');
    } else {
      notify.error('Gagal publish', res.error);
    }
  };

  const handleUnpublish = async (id) => {
    const res = await scheduleService.setEventStatus(id, 'draft');
    if (res.success) {
      setItems(items.map(i => i.id === id ? { ...i, status: 'draft' } : i));
      notify.info('Draft', 'Jadwal disembunyikan dari website utama.');
    } else {
      notify.error('Gagal unpublish', res.error);
    }
  };
                  ...
                  {filteredItems.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        {item.status === 'draft' ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase bg-gray-100 border border-gray-300 text-gray-600">
                            <EyeOff className="h-3 w-3" /> Draft
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase bg-emerald-50 border border-emerald-200 text-emerald-700">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {item.status === 'draft' ? (
                            <button
                              ...
                              className="p-1.5 text-emerald-600 hover:bg-emerald-50 border border-transparent hover:border-emerald-200 rounded-lg transition-colors"
                            ...
                            <button
                              ...
                              className="p-1.5 text-yellow-600 hover:bg-yellow-50 border border-transparent hover:border-yellow-200 rounded-lg transition-colors"
                ...
                <div key={item.id} className="p-4 flex flex-col gap-3 hover:bg-gray-50/50 transition-colors">
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    {item.status === 'draft' ? (
                      <button
                        onClick={() => handlePublish(item.id)}
                        className="px-3 py-1.5 text-xs font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg flex items-center gap-1.5"
                      >
                      ...
                      <button
                        onClick={() => handleUnpublish(item.id)}
                        className="px-3 py-1.5 text-xs font-bold text-yellow-600 bg-yellow-50 border border-yellow-100 rounded-lg flex items-center gap-1.5"
                      >
