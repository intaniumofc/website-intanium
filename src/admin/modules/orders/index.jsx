...
export default function AdminOrdersPage() {
  ...
  async function executeSingleStatusUpdate(order, nextStatus) {
    ...
    setError('');
    try {
      const updated = await orderService.updateOrderStatus(order.id, nextStatus, statusAuditNote);
      setOrders((current) =>
    ...
    try {
      const results = await Promise.all(
        orderIds.map(async (orderId) =>
          orderService.updateOrderStatus(orderId, status, statusAuditNote),
        ),
      );
      setOrders((current) =>
        current.map((order) => {
  ...
  async function handleUpdateShippingCost() {
    ...
    try {
      const updated = await orderService.updateOrderShippingCost(detailData.order.id, cost);
  ...
  async function handleSaveFulfillment() {
    ...
    setFeedback('');
    try {
      const updated = await orderService.updateOrderFulfillment(detailData.order.id, {
