...
export default function HomeHeroSection() {
  ...
  // Play video reliably when mounted and when preloader completes
  useEffect(() => {
    ...
    const playVideo = () => {
      video.play().catch((err) => {
        console.warn('[HomeHeroSection] Video play fallback:', err);
      });
    };
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      if (scrollIndicatorRef.current) tl.to(scrollIndicatorRef.current, { opacity: 0, y: -20, duration: 0.3 }, 0);
      if (videoContainerRef.current) tl.to(videoContainerRef.current, { scale: 1.08, ease: 'none', duration: 3 }, 0);
      if (noiseRef.current) tl.to(noiseRef.current, { opacity: 0.07, duration: 2 }, 0);
      if (titleRef.current) tl.to(titleRef.current, { y: -100, opacity: 0, duration: 1.2, ease: 'power2.inOut' }, 0.8);
      if (subtitleRef.current) tl.to(subtitleRef.current, { y: -80, opacity: 0, duration: 1, ease: 'power2.inOut' }, 0.9);
      if (ctaRef.current) tl.to(ctaRef.current, { y: -60, opacity: 0, duration: 0.8, ease: 'power2.inOut' }, 1.0);
    }, container);
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-6 text-center">
        ...
        <div ref={ctaRef} className="mt-8 flex items-center gap-4 flex-wrap justify-center">
          ...
          <Link
            ...
            className="group inline-flex items-center gap-2.5 px-7 py-3.5 rounded-full border border-white/30 bg-white/15 text-white font-bold text-xs uppercase tracking-[0.15em] hover:bg-white/25 hover:scale-105 active:scale-95 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2"
          >
            Tentang Kami
