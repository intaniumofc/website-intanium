...
const ScheduleSection = () => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] justify-between gap-8 md:gap-12 items-start mt-8">

        {/* Left Panel: Mini Calendar */}
        <motion.div variants={fadeUp} className="bg-[#fdfcf8] p-5 rounded-3xl border border-(--border-color) shadow-(--box-shadow-sm) mx-auto w-full max-w-sm lg:max-w-none">
          {/* Header */}
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-black text-(--color-primary) font-heading">
              {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
            </h3>
            <div className="flex gap-2">
              <button onClick={prevMonth} className="p-2 hover:bg-slate-100 rounded-full transition-colors cursor-pointer text-slate-500 hover:text-(--color-primary)">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={nextMonth} className="p-2 hover:bg-slate-100 rounded-full transition-colors cursor-pointer text-slate-500 hover:text-(--color-primary)">
              ...
              return (
                <div key={i} className="flex flex-col items-center justify-center">
                  <button
                    ...
                    className={`w-8 h-8 flex items-center justify-center rounded-full text-[13px] font-bold transition-all duration-300 cursor-pointer ${isSelected
                  ...
                  <div className="flex gap-0.75 mt-1 h-1.5 items-center justify-center">
                    {getEventsForDate(date).length > 0 && [...new Set(getEventsForDate(date).map(e => e.type))].map((type, idx) => (
                      <div key={idx} className={`w-1.5 h-1.5 rounded-full ${getDotColor(type)}`}></div>
                    ))}
                  </div>
                </div>
              );
        ...
        <motion.div variants={fadeUp} className="flex flex-col h-full">
          <div className="mb-6 border-b border-(--border-color) pb-4">
            <h3 className="text-2xl font-black text-(--color-primary) font-heading">
          ...
          <div className="space-y-3">
            {selectedEvents.length > 0 ? (
              selectedEvents.map((event, idx) => (
                <div key={idx} className="group p-4 rounded-2xl border border-(--border-color) bg-white hover:shadow-sm transition-all duration-300 flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
          ...
          {upcomingEvents.length > 0 && selectedEvents.length <= 1 && (
            <div className="mt-8 pt-6 border-t border-(--border-color)">
              <h4 className="text-xs font-bold text-slate-400 tracking-widest mb-4">Upcoming</h4>
              <div className="space-y-3">
                {upcomingEvents.map((evt, idx) => {
                  const dateObj = new Date(evt.time.replace(' WIB', '').replace(/:/g, 'T')); // Not reliable for string manipulation, using evt.date directly
                  const day = evt.date.split('-')[2];
                  const monthIdx = parseInt(evt.date.split('-')[1]) - 1;
                  return (
                    <div key={idx} onClick={() => setSelectedDate(new Date(evt.date))} className="flex items-center justify-between group cursor-pointer bg-white/30 hover:bg-white/60 p-3 rounded-xl border border-transparent hover:border-(--border-color) transition-all duration-300">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex flex-col items-center justify-center text-(--color-primary)">
