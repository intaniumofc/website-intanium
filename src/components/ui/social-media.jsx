...
const SocialTooltip = React.forwardRef(({ className, items, ...props }, ref) => {
  // Base styles for the component - Pink background and dark pink icon color when not hovered
  const baseIconStyles =
    "relative flex items-center justify-center w-9.5 h-9.5 sm:w-10 sm:h-10 rounded-full overflow-hidden transition-all duration-300 ease-in-out group-hover:shadow-lg hover:scale-105 active:scale-95";
        ...
        return (
          <li key={index} className="relative group list-none">
            ...
            >
              ...
              {/* Icon rendering (Pink color by default, white on hover) */}
              <div className="relative z-10 transition-colors duration-300 ease-in-out text-[var(--color-pink-dark)] group-hover:text-white flex items-center justify-center">
                {hasCustomIcon ? (
